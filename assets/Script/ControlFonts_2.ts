...
export default class NewClass extends cc.Component {
    ...
    update (dt) {
        if(this.texto_1.enabled && this.onclick == 1){
            ...
            if(this.citysRemaining == 1 && !this.texto_1.enabled && this.activartexto2){
        ...
        if(this.texto_2.getComponent(cc.RichText).enabled && this.texto_2.enabled){
          
            this.time -= dt;
        }

        if(this.time <= 0 && this.texto_2.getComponent(cc.RichText).enabled){
            
            this.activartexto2 = false;
            this.texto_2.enabled = false;
            this.texto_3.getComponent(cc.RichText).enabled = true;
            this.time = 5;
        }

        if(this.texto_3.getComponent(cc.RichText).enabled){
          
            this.time -= dt;
        }

        if(this.time <= 0 && this.texto_3.getComponent(cc.RichText).enabled){
           
            this.texto_3.enabled = false;
        }
    }
